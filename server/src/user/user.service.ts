...
const generateAccessToken = (id: Types.ObjectId, username: string, name: string, surname: string, avatar: string) => {
    const payload = {
        id: id,
        username: username,
        name: name,
...
export class UserService {
    constructor(@InjectModel(User.name) private userModel: Model<User>, @InjectModel(Contact.name) private contactModel: Model<Contact>, private fileService: FileService) {}
    ...
    async updateToken(dto: UpdateTokenDto) {
        const token = generateAccessToken(dto.id, dto.username, dto.name, dto.surname, dto.avatar);
        return JSON.parse(`{"token": "${token}"}`);
    }
